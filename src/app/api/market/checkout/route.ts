 ...
 *
 * L'INTERA funzione sta dentro un `try`. Prima le prime query — lettura
 * dell'artefatto, controllo di un acquisto gia' in corso — erano fuori: se
 * una falliva, Next.js rispondeva 500 SENZA corpo, e all'utente arrivava un
 * nudo «HTTP 500». Un errore che non dice niente e' peggio di un errore
 * brutto: non si sa nemmeno da che parte cercare.
 */
  ...
  try {
    ...
    if (/column|relation|does not exist|violates|constraint/i.test(testo)) {
      return NextResponse.json({
        error: 'Il database del sito non e\' pronto per gli acquisti.',
        detail: testo + '. Esegui marketplace_schema.sql sul Postgres.',
        raw: testo,
      }, { status: 500 });
    }
    return NextResponse.json({
      error: 'Errore del sito durante l\'acquisto.',
      detail: testo, raw: testo,
    }, { status: 500 });
  }
}

async function gestisci(req: NextRequest) {
  ...
  const a = await pool.query(
    `SELECT a.id, a.name, a.summary, a."priceCents", a.currency, a."pricePeriod",
            a."isForSale", a."ownerId" AS "authorId", a.delivery::text AS delivery,
            s."providerRef" AS "sellerAcct", s."payoutsEnabled"
       FROM "Artifact" a
       LEFT JOIN "SellerAccount" s ON s."userId" = a."ownerId"
  ...
  // Solo acquisto una tantum. L'abbonamento non e' implementato: rifiutare qui
  // e' meglio che incassare qualcosa che poi nessuno sa rinnovare o revocare.
  if (String(art.pricePeriod || 'once') !== 'once') {
    return NextResponse.json(
      { error: 'i pagamenti ricorrenti non sono ancora attivi: questo artefatto non e\' acquistabile' },
      { status: 400 });
  }
  if (!configurato()) {
  ...
  // ── PAGAMENTO GIA' AVVIATO ─────────────────────────────────────────────
  // Si controllava solo la LICENZA. Ma fra il pagamento e la licenza c'e' il
  ...
  if (att) {
    // 'paid' senza licenza significa che il webhook e' arrivato a meta': si
    // dice, invece di far pagare di nuovo.
    return NextResponse.json({
      ok: false,
      inAttesa: true,
      error: att.status === 'paid'
        ? 'Il pagamento risulta incassato, ma l\'accesso non e\' ancora aperto.'
        : 'Hai gia\' un pagamento in corso per questo artefatto.',
      detail: att.status === 'paid'
        ? 'Stripe ha confermato l\'incasso ma la licenza non e\' stata creata: '
  ...
  try {
    ...
    // Un errore del DATABASE e un errore di STRIPE si somigliano da fuori —
    // entrambi 500 — ma hanno cause opposte: uno e' uno schema incompleto,
    // l'altro un problema di configurazione dei pagamenti. Dirlo evita di
    // cercare dalla parte sbagliata.
    if (/column|relation|violates|constraint/i.test(testo)) {
      return NextResponse.json({
        error: 'Il database del sito non e\' pronto per gli acquisti.',
        detail: 'Manca qualcosa nella tabella Purchase: ' + testo
              + '. Esegui marketplace_schema.sql sul Postgres.',
        raw: testo,
      }, { status: 500 });
    }
    return NextResponse.json({ error: testo, raw: testo }, { status: st });
  }
}
