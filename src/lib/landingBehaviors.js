/* Northrock — Landing page behavior (vanilla port of the original component logic).
  ...
  for (let i = 0; i < missing.length; i += size) {
    ...
    try {
      const out = await window.claude.complete({
        max_tokens: 8000,
        system: 'You are a professional UI localizer for a fintech web app. Translate each string from Italian into ' + names[lang] + '. Preserve meaning and tone; keep the product name "LYRA", tickers, numbers and punctuation intact. Return ONLY a JSON array of strings — same length and order as the input, translations only, no commentary.',
...
export function run(list) {
  const go = () => { root = document.getElementById('app-root'); if (!root) return; list.forEach((name) => { const fn = REGISTRY[name]; if (fn) { try { fn(); } catch (e) { console.warn('[qs] ' + name, e); } } }); };
